...
# plot function ----------------------------------------------------------------------
  coef_v2 <- function(dat, title, leg_lab, leg_col, leg_shp, x_mn, x_mx, brks){ 
    ...
    ggsave(save, plot = last_plot(), height = 12, width = 13, units = "cm", dpi = 500, device = "jpeg")
    return(q) 
...
# C3 native and nonnative NZ, native AU
  coef_v2(d,
          ...
          c("Nonnative NZ", "Native NZ", "Native AU"),
          c("red", "grey35","grey35"),
          leg_shp <- c(19, 19, 17),
          x_mn <- -1.4,
          x_mx <- 2,
...
# C4 Aus native-NZ nonnative (e)
  coef_v2(e,
          ...
          leg_lab <- c("Nonnative NZ", "Native AU"),
