...
temp_rast <- rast(xmin = 110, xmax = 155, ymin = -45, ymax = -10, 
                  resolution = 5/6)
values(temp_rast) <- 1:ncell(temp_rast)
plot(temp_rast)

source("RScripts/7. GAM/01-create_aus_raster.R")
source("RScripts/7. GAM/02-estimate_species_richness_Aus.R")
...
# read in richness data from iNEXT
  rich_nat_C3 <- read.csv("Results/csv/species richness/rich_natC3.csv")
  rich_nat_C4 <- read.csv("Results/csv/species richness/rich_natC4.csv")
  rich_exo_C3 <- read.csv("Results/csv/species richness/rich_exoC3.csv")
  rich_exo_C4 <- read.csv("Results/csv/species richness/rich_exoC4.csv")
...
# full raster
env_rast <- c(env_rast, rich)
names(env_rast)

# create data frame
env <- as.data.frame(env_rast, cells = T, na.rm = F)
...
cr <- data.frame( 
  var = names(env[2:8]),
  natc3 = as.vector(cor(envh1$nat_C3, envh1[, 2:8], method = "spearman", use = "pairwise.complete.obs")),
  exoc3 = as.vector(cor(envh1$exo_C3, envh1[, 2:8], method = "spearman", use = "pairwise.complete.obs")),
  natc4 = as.vector(cor(envh1$nat_C4, envh1[, 2:8], method = "spearman", use = "pairwise.complete.obs")),
  exoc4 = as.vector(cor(envh1$exo_C4, envh1[, 2:8], method = "spearman", use = "pairwise.complete.obs")))
...
pdf("Results/figures/Figure S1.pdf")
...
fit <- function(var) {
  dat <- env
  dat$rich <- var 
  m_formula <- formula(rich ~ s(area) + s(amt) + s(arid) + s(ts) + s(pwarmq) + 
                         s(pcoldq) + s(th) + s(hii) + s(long, lat))
  mod <- gam(m_formula, data = dat, method = "ML")
  # removed pewc
  return(mod)
}

m_nat_C3 <- fit(log(env$nat_C3))
m_nat_C4 <- fit(log(env$nat_C4))
m_exo_C3 <- fit(log(env$exo_C3))
m_exo_C4 <- fit(log(env$exo_C4))

# new raster to store the modelled results
# predict to all cells
predto <- env[, c(2:9, 14:15)] # was 2:10, 15:16
...
pred_nat_C3 <- pr(m_nat_C3)
pred_nat_C4 <- pr(m_nat_C4)
pred_exo_C3 <- pr(m_exo_C3)
pred_exo_C4 <- pr(m_exo_C4)
...
pdf("Results/figures/Figure 2.pdf")
...
###############################################################################
pdf("Results/figures/Figure 3.pdf")
...
snc3 <- summary(m_nat_C3)  
snc4 <- summary(m_nat_C4)  
sec3 <- summary(m_exo_C3)  
sec4 <- summary(m_exo_C4)  
