...
source("RScripts/Create predictor variable raster.R")
source("RScripts/Estimate species richness.R")

#-------------------------------------------------------------------------------------

# read in environment raster
env_rast <- rast("Data files/Environment/Environment raster.tif")
env_rast
names(env_rast)
...
aus <- st_read("AUS_2021_AUST_SHP_GDA2020/AUS_2021_AUST_GDA2020.shp")
...
rich_nat_C3 <- read.csv("Results/csv/rich_natC3.csv")
rich_nat_C4 <- read.csv("Results/csv/rich_natC4.csv")
rich_exo_C3 <- read.csv("Results/csv/rich_exoC3.csv")
rich_exo_C4 <- read.csv("Results/csv/rich_exoC4.csv")
...
genras <- function(dat) {
  dat <- dat %>%
    filter(tot.rec >= 50 & warn == 0 & method != "observed")
...
n.miss <- table(nm <- apply(env, 1, function(x) sum(is.na(x[2:10]))))
n.miss
...
cr <- data.frame(
  var = names(env[2:9]),
  natc3 = as.vector(cor(envh1$nat_C3, envh1[, 2:9], method = "spearman", use = "pairwise.complete.obs")),
  exoc3 = as.vector(cor(envh1$exo_C3, envh1[, 2:9], method = "spearman", use = "pairwise.complete.obs")),
  natc4 = as.vector(cor(envh1$nat_C4, envh1[, 2:9], method = "spearman", use = "pairwise.complete.obs")),
  exoc4 = as.vector(cor(envh1$exo_C4, envh1[, 2:9], method = "spearman", use = "pairwise.complete.obs")))
...
fit <- function(var) {
  ...
  m_formula <- formula(rich ~ s(area) + s(amt) + s(arid) + s(ts) + s(pwarmq) + 
                         s(pcoldq) + s(pewc) + s(th) + s(hii) + s(long, lat))
